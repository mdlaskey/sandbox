...
open("Net/hdfalex/train.txt", 'w').close()
open("Net/hdfalex/test.txt", 'w').close()
train_writer = open("Net/hdfalex/train.txt", "w+")
test_writer = open("Net/hdfalex/test.txt", "w+")
...
for dataset_name in dataset_names:
    ...
    for line in controls_reader:
        ...
        if segment:
            ...
            im = bc.pipe(im, h=227, w=227)
            cv2.imwrite(Options.data_dir + "images_alex/" + new_filename, im)

        controls_string = Dataset.controls2str(controls)
        line = Options.data_dir + "images_alex/" + new_filename + controls_string + "\n"
...
hdfalex.img2hdf('./Net/hdfalex/train')

print "converting test set to hdf..."
hdfalex.img2hdf('./Net/hdfalex/test')
